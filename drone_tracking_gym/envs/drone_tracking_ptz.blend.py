...
path.append("E:/lcc/project_space/UAV-tracking-ftz/cranfield-drone-tracking-gym-main/drone_tracking_gym/envs")
#Internal libs
from DroneControl import DroneControl, DroneControl_DynamicTracking, DroneControl_ObstacleTracking
from CameraControl import CameraControl
from OffscreenRenderer import OffScreenRenderer2
...
class DroneTrackingEnv_Base(btb.env.BaseEnv):
    ...
    def __init__(self, agent, drone_name, camera_name, dt, scenario, scenario_length = 300):
        ...
        self.renderer = OffScreenRenderer2(camera_name, mode="rgb")
...
#         self.display_stats = False
#         self.renderer = OffScreenRenderer2(camera_name, mode="rgba")
#         RENDER_STYLE = "MATERIAL"
#         #RENDER_STYLE = "RENDERED"
#         self.renderer.set_render_style(RENDER_STYLE, True)
...
class DroneTrackingEnv_PTZ(DroneTrackingEnv_Base):

    def _env_post_step(self):

        result = super()._env_post_step()

        pan  = self.camera.camera_object.rotation_euler[2]   #  (yaw)
        tilt = self.camera.camera_object.rotation_euler[0]   # (pitch)
        zoom = self.camera.camera.lens                        # 

        result['ptz'] = (pan, tilt, zoom)
        return result
...
def main():
    ...
    env = DroneTrackingEnv_PTZ(agent, "Sphere", "Camera", 1/fps, DRONE_SCENARIO)
